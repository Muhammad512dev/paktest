...
const normalizeText = (v: any) => String(v || '').trim().toLowerCase().replace(/\s+/g, ' ');

const resolveChoiceIndex = (raw: any, opts: any[]): number => {
  ...
  if (m3?.[1]) return ['a','b','c','d','e','f','g','h'].indexOf(m3[1].toLowerCase());

  // Numeric index
  if (/^\d+$/.test(compact)) {
    const idx = parseInt(compact, 10);
    if (Number.isFinite(idx) && idx >= 0 && idx < (opts?.length || 0)) return idx;
  }

  // Exact option text match
  const byText = (opts || []).findIndex((opt: any) => normalizeText(opt) === norm);
  if (byText !== -1) return byText;

  return -1;
};

const isPaperAttemptOpen = (paper: any, now = new Date()): boolean => {
  if (!paper?.examDate) return true; // no schedule => allow
...
const authenticateStudent = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    if (!enabled) return res.status(403).json({ error: 'Online Test feature is not enabled for your school package. Please upgrade to continue.' });
...
router.post('/login', async (req: any, res: any) => {
  ...
  try {
    ...
    if (!enabled) return res.status(403).json({ error: 'Online Test feature is not enabled for your school package. Please contact your school admin to upgrade.' });
    ...
    const token = jwt.sign(
      { 
        ...
        className: (student as any).classLevel?.name || '',
        name: student.name,
...
router.get('/papers', authenticateStudent, async (req: any, res: any) => {
  try {
    ...
    const now = new Date();
    const papers = (papersAll || []).filter((p: any) => isPaperAttemptOpen(p, now));
    res.json(papers);
  } catch (e) {
...
// --- SAVE/UPDATE DRAFT ---
router.post('/draft', authenticateStudent, async (req: any, res: any) => {
...
// --- FETCH DRAFT ---
router.get('/draft/:paperId', authenticateStudent, async (req: any, res: any) => {
  try {
...
const papersCache = new Map<string, any>();

// --- FETCH PAPER DETAILS (SHUFFLED) ---
router.get('/papers/:id', authenticateStudent, async (req: any, res: any) => {
  try {
    ...
    // Shuffle questions
    let questions = paper.questions as any[];
...
// --- SUBMIT EXAM ---
router.post('/submit', authenticateStudent, async (req: any, res: any) => {
  ...
  try {
    ...
    const paperQuestions = paper.questions as any[];
    let totalScore = 0;
    ...
    for (const q of paperQuestions) {
        ...
        if (isObjective && studentAns !== undefined && studentAns !== null && studentAns !== '') {
            ...
            if (q.type === 'MCQ') {
                ...
                let opts: string[] = [];
                try {
                    if (typeof q.options === 'string') opts = JSON.parse(q.options);
                ...
                if (isCorrect) autoScore = q.marks;
                ...
                const studentPairs: any[] = Array.isArray(studentAns) ? studentAns : [];
                const correctPairs: any[] = Array.isArray(q.matchingPairs) ? q.matchingPairs : [];
                ...
                if (studentPairs.length > 0 && correctPairs.length > 0) {
                    ...
                    correctPairs.forEach((p: any) => {
                        if (p.left) correctMap[normalize(p.left)] = normalize(p.right);
                    });

                    const allCorrect = studentPairs.every((sp: any) => {
    ...
    res.status(500).json({ error: 'Submission failed', details: (e as any).message });
  }
});

// --- GET STUDENT RESULTS ---
router.get('/results', authenticateStudent, async (req: any, res: any) => {
    try {
        ...
        if (results.length > 0) {
            ...
            if (results[0].paper?.questions) {
                ...
                console.log(`[DEBUG] Questions length/keys: ${Array.isArray(qs) ? qs.length : typeof qs === 'string' ? qs.length : Object.keys(qs as any).length}`);
            }
        }
        ...
        const processedResults = results.map((r: any) => {
            ...
            if (typeof paperQuestions === 'string') {
                try { questionsArr = JSON.parse(paperQuestions); } catch(e) { questionsArr = []; }
            } else {
                questionsArr = Array.isArray(paperQuestions) ? paperQuestions : Object.values(paperQuestions || {});
            }

            const hasSubjective = questionsArr.some((q: any) => 
                !['MCQ', 'Match Columns', 'Fill in the Blanks', 'True/False'].includes(q.type)
            );

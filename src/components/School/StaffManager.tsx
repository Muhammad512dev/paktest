...
import { getStaff, addStaff, updateStaff, deleteStaff, getSchoolById, getPlans, getSyllabuses, getClasses, getSubjects } from '../../services/dataService';
import { Staff, User, Syllabus, ClassLevel, Subject } from '../../types';
...
const StaffManager: React.FC<StaffManagerProps> = ({ user }) => {
  ...
  const loadStaffData = async () => {
    ...
    try {
      ...
      if (user?.schoolId) {
          ...
          const currentPlan = plans.find((p: any) => p.name === schoolData.subscriptionPlan);
      ...
      // 3. Load Syllabuses, Classes, Subjects
      const [syllabusList, classList, subjectList] = await Promise.all([
        getSyllabuses(),
        getClasses(),
        getSubjects()
      ]);
    ...
    try {
      if (editingStaff.id) {
        ...
        const newStaff: any = { // Cast to any to include password
          ...
          assignedSyllabuses: editingStaff.subjects || [],
          assignedClasses: editingStaff.assignedClasses || [],
          assignedSubjects: editingStaff.assignedSubjects || [],
  ...
  const toggleSubject = (subjectName: string) => {
    ...
    if (currentSubjects.includes(subjectName)) {
      setEditingStaff({ ...editingStaff, subjects: currentSubjects.filter(s => s !== subjectName), assignedSyllabuses: currentSubjects.filter(s => s !== subjectName) });
    } else {
      setEditingStaff({ ...editingStaff, subjects: [...currentSubjects, subjectName], assignedSyllabuses: [...currentSubjects, subjectName] });
    }
  };

  const toggleStaffClass = (classId: string) => {
    const current = editingStaff.assignedClasses || [];
    if (current.includes(classId)) {
      setEditingStaff({ ...editingStaff, assignedClasses: current.filter(id => id !== classId) });
    } else {
      setEditingStaff({ ...editingStaff, assignedClasses: [...current, classId] });
    }
  };

  const toggleStaffSubject = (subjectId: string) => {
    const current = editingStaff.assignedSubjects || [];
    if (current.includes(subjectId)) {
      setEditingStaff({ ...editingStaff, assignedSubjects: current.filter(id => id !== subjectId) });
    } else {
      setEditingStaff({ ...editingStaff, assignedSubjects: [...current, subjectId] });
    }
  };
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                     ...
                     <div className="space-y-2">
                        <p className="text-xs font-bold text-gray-500 uppercase">Assigned Boards / Syllabuses</p>
                        <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto border border-gray-100 p-3 rounded-xl bg-gray-50">
                           {syllabuses.map(syl => (
                              <button
                                 key={syl.id}
                                 type="button"
                                 onClick={() => toggleSubject(syl.id)}
                                 className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all flex flex-col items-start gap-1 ${
                                    editingStaff.subjects?.includes(syl.id)
                     ...
                     {editingStaff.role === 'TEACHER' && editingStaff.subjects && editingStaff.subjects.length > 0 && (
                       <>
                         <div className="space-y-2">
                            <p className="text-xs font-bold text-gray-500 uppercase">Assigned Classes</p>
                            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto border border-gray-100 p-3 rounded-xl bg-gray-50">
                               {allClasses.filter(c => editingStaff.subjects?.includes(c.syllabusId)).map(cls => (
                                  <button
                                     key={cls.id}
                                     type="button"
                                     onClick={() => toggleStaffClass(cls.id)}
                                     className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all flex flex-col items-start gap-1 ${
                                        editingStaff.assignedClasses?.includes(cls.id)
                                        ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg'
                                        : 'bg-white border-gray-200 text-gray-600 hover:border-indigo-300'
                                     }`}
                                  >
                                     ...
                                     <span className="text-[10px] opacity-70 font-medium">
                                       {syllabuses.find(s => s.id === cls.syllabusId)?.name}
                                     </span>
                                  </button>
                               ))}
                            </div>
                         ...
                         <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto border border-gray-100 p-3 rounded-xl bg-gray-50">
                               {allSubjects.filter(s => 
                                  editingStaff.subjects?.includes(s.syllabusId) && 
                                  (!editingStaff.assignedClasses || editingStaff.assignedClasses.length === 0 || editingStaff.assignedClasses.includes(s.classId))
                               ).map(sub => (
                                  <button
                                     key={sub.id}
                                     type="button"
                                     onClick={() => toggleStaffSubject(sub.id)}
                                     className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all flex flex-col items-start gap-1 ${
                                        editingStaff.assignedSubjects?.includes(sub.id)
                                        ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg'
                                        : 'bg-white border-gray-200 text-gray-600 hover:border-indigo-300'
                                     }`}
                                  >
                                     {sub.name}
                                     <span className="text-[10px] opacity-70 font-medium">
                                       {allClasses.find(c => c.id === sub.classId)?.name}
                                     </span>
                                  </button>
                               ))}
                            </div>

...
import { getSchoolById, getPapersBySchool, getNotificationsForSchool, getTeacherStats } from '../services/dataService';
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, user }) => {
  ...
  const stats = [
    { label: 'Total Papers', value: (user.role === 'TEACHER' ? teacherStats?.papersCount : school?.stats.papersCount)?.toLocaleString() || '0', trend: '+12%', icon: Files, color: 'text-brand', bg: 'bg-brand-light' },
    { label: 'Active Students', value: (user.role === 'TEACHER' ? teacherStats?.studentCount : school?.stats.studentCount)?.toLocaleString() || '0', trend: '+5%', icon: Users, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Avg. Score', value: (user.role === 'TEACHER' ? teacherStats?.avgScore : '78%') || '0%', trend: '+2%', icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'Submissions', value: (user.role === 'TEACHER' ? teacherStats?.submissionsCount : school?.stats.teachersCount)?.toString() || '0', trend: 'Stable', icon: Users, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100/80 shadow-sm hover:shadow-md hover:translate-y-[-2px] transition-all duration-300 cursor-default">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Paper Title</th>
                  <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Subject / Class</th>
                ...
                {recentPapers.map((paper) => (
                  <tr key={paper.id} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-6 py-4">
                       <div className="font-bold text-sm text-slate-900 group-hover:text-brand transition-colors">{paper.title}</div>
                       <div className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">Author: {paper.author}</div>
                    </td>
                    <td className="px-6 py-4">
                       <div className="text-sm font-medium text-slate-700">{paper.subject}</div>
                       <div className="text-xs text-slate-400">{paper.classLevel}</div>
                    </td>
                    <td className="px-6 py-4 text-xs font-medium text-slate-500 whitespace-nowrap">{paper.dateCreated}</td>
                    <td className="px-6 py-4">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-indigo-950 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <button 
                onClick={() => onNavigate('billing')}
                className="w-full py-3 bg-white text-slate-900 text-xs font-extrabold uppercase tracking-[0.2em] rounded-2xl hover:bg-brand hover:text-white transition-all shadow-lg"
              >
                Contract Details
              </button>
            </div>

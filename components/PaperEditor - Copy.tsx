...
import { 
  ...
  Calendar, Clock, ClipboardList, Info as InfoIcon, CheckSquare, Square, ChevronDown, Globe, Languages, GraduationCap, ChevronRight, ChevronUp, BookOpen, Settings2, HelpCircle, Eye, EyeOff
} from 'lucide-react';
...
const PaperEditor: React.FC<PaperEditorProps> = ({ paper, onBack, user }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex bg-white overflow-hidden print:static">
      ...
      {isConfigModalOpen && sectionConfig && (
        <div className="fixed inset-0 z-[250] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                 ...
                 <button onClick={() => setIsConfigModalOpen(false)} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X size={20}/></button>
              </div>
              <div className="p-8 space-y-6">
                 ...
                 <div>
                    ...
                    <select 
                       value={sectionConfig.questionType} 
                       onChange={e => setSectionConfig({...sectionConfig, questionType: e.target.value})} 
                       className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                       {availableQuestionTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                    </select>
                 </div>
                 <div className="grid grid-cols-3 gap-4">
                    <div>
                       <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Required Qs</label>
                       <input 
                          type="number" 
                          min="1"
                          value={sectionConfig.selectCount} 
                          onChange={e => {
                             const newSelect = parseInt(e.target.value) || 0;
                             setSectionConfig({
                                 ...sectionConfig, 
                                 selectCount: newSelect,
                                 totalCount: newSelect + (sectionConfig.totalCount - sectionConfig.selectCount) 
                             })
                          }} 
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                       />
                    </div>
                    <div>
                       <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Choice/Ignore</label>
                       <input 
                          type="number" 
                          min="0"
                          value={sectionConfig.totalCount - sectionConfig.selectCount} 
                          onChange={e => {
                             const newIgnore = parseInt(e.target.value) || 0;
                             setSectionConfig({
                                 ...sectionConfig, 
                                 totalCount: sectionConfig.selectCount + newIgnore
                             })
                          }} 
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                       />
                    ...
                    <div>
                       <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Marks Each</label>
                       <input 
                          type="number" 
                          min="0.5"
                          step="0.5"
                          value={sectionConfig.marksPerQuestion} 
                          onChange={e => setSectionConfig({...sectionConfig, marksPerQuestion: parseFloat(e.target.value) || 0})} 
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-200 rounded-xl font-bold text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                       />
                    </div>
                 </div>
                 <div className="p-4 bg-indigo-50 rounded-2xl flex items-start gap-3 border border-indigo-100">
                    <HelpCircle size={18} className="text-indigo-600 mt-0.5" />
                    <p className="text-[10px] text-indigo-900 font-bold uppercase tracking-tight leading-relaxed">
                       Configuration: <strong>{sectionConfig.selectCount}</strong> Required + <strong>{sectionConfig.totalCount - sectionConfig.selectCount}</strong> Choice = <strong>{sectionConfig.totalCount}</strong> Total Questions in pool.
                    </p>
                 </div>
              </div>
              <div className="p-6 border-t border-gray-100 bg-gray-50 flex gap-3">
                 <button onClick={() => setIsConfigModalOpen(false)} className="flex-1 py-3 text-sm font-bold text-gray-400 hover:bg-gray-200 rounded-xl transition-all">Cancel</button>
                 <button 
      ...
      <aside className="w-80 bg-slate-900 flex flex-col h-full shrink-0 border-r border-slate-800 z-50 print:hidden relative shadow-2xl overflow-y-auto">
         ...
         <div className="flex-1 p-6 space-y-8">
            ...
            <div className="space-y-3 pt-6 border-t border-slate-800">
                ...
                <button onClick={() => setShowAnswers(!showAnswers)} className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${showAnswers ? 'bg-emerald-600/20 border-emerald-600 text-emerald-300' : 'bg-slate-800/50 border-slate-700 text-slate-400'}`}>
                    <span className="text-[10px] font-bold uppercase">Show Answers (Inline)</span>
                    {showAnswers ? <CheckSquare size={14}/> : <Square size={14}/>}
                </button>
            </div>

...
import { 
  ...
  getClasses, addClass, deleteClass, updateSyllabus, updateClass,
...
const CurriculumManager: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<{ id: string; type: string; name: string; description?: string; logo?: string } | null>(null);
  ...
  const handleAddItem = async () => {
    ...
    if (editingItem) {
      ...
      if (editingItem.type === 'classes') data.syllabusId = multiSelSyllabusIds[0];
  ...
  const openEditModal = (type: string, item: any) => {
    ...
    setNewItemImage(null);
    setRemoveExistingImage(false);
    setSelSyllabusId(item.syllabusId || '');
    setSelClassId(item.classId || '');
    setSelSubjectId(item.subjectId || '');
    setMultiSelSyllabusIds(item.syllabusId ? [item.syllabusId] : []);
    setMultiSelClassIds(item.classId ? [item.classId] : []);
    setMultiSelSubjectIds(item.subjectId ? [item.subjectId] : []);
    setMultiSelChapterIds(item.chapterId ? [item.chapterId] : []);
    if (type === 'topics') {
      const chapter = chapters.find(ch => ch.id === item.chapterId);
      const subject = subjects.find(s => s.id === chapter?.subjectId);
      setSelSubjectId(chapter?.subjectId || '');
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
              <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                 <div>
                    <h3 className="font-bold text-xl text-gray-900 flex items-center gap-2">
                       ...
                       {editingItem ? 'Edit' : 'Add'} {activeTab.toLowerCase()}
                    </h3>
                    ...
                    <div className="space-y-3">
                       ...
                       <div className="p-5 border border-indigo-50 rounded-3xl bg-indigo-50/20 flex items-center gap-6">
                          <div className="w-20 h-20 bg-white border-2 border-dashed border-indigo-200 rounded-2xl flex items-center justify-center overflow-hidden relative group cursor-pointer shadow-sm">
                             {newItemImage ? <img src={URL.createObjectURL(newItemImage)} className="w-full h-full object-contain" /> : (editingItem?.logo && !removeExistingImage ? <img src={editingItem.logo} className="w-full h-full object-contain" /> : <ImageIcon size={32} className="text-indigo-200"/>)}
                             <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" onChange={e => { setNewItemImage(e.target.files?.[0] || null); setRemoveExistingImage(false); }} />
                          </div>
                          <div>
                             ...
                             {(newItemImage || (editingItem?.logo && !removeExistingImage)) && <button onClick={() => { setNewItemImage(null); setRemoveExistingImage(true); }} className="mt-2 text-[10px] font-bold text-red-500 uppercase flex items-center gap-1"><Trash size={10}/> Clear</button>}
                          </div>
                       </div>
                    </div>
                 )}
              ...
              <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
                 ...
                 <button disabled={!newItemName} onClick={handleAddItem} className="px-12 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all disabled:opacity-50">{editingItem ? 'Save Changes' : 'Commit to Repository'}</button>
              </div>
           </div>
        </div>
      )}

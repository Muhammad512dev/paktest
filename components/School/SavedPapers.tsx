...
import { getPapersBySchool, deletePaper } from '../../services/dataService';
...
const SavedPapers: React.FC<SavedPapersProps> = ({ user }) => {
  ...
  const handlePrint = (paper: any) => {
      // Cast to ExamPaper as the API returns full object structure
      setPrintingPaper(paper as ExamPaper);
  };

  const handleEdit = (paper: any) => {
      // Prepare paper object for editor, ensuring selectedChapters/Topics arrays exist
      // even if backend didn't persist them (UI state).
      const editorPaper = {
          ...paper,
          selectedChapters: paper.selectedChapters || [],
          selectedTopics: paper.selectedTopics || []
      };
      setEditingPaper(editorPaper);
  };
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <div className="text-xs font-bold text-gray-700 flex items-center gap-2">
                        <Calendar size={12} className="text-slate-400" /> {paper.examDate || 'Not Set'}
                      </div>
                      <div className="text-[10px] font-bold text-gray-400 flex items-center gap-2 uppercase">
                        <Clock size={12} className="text-slate-400" /> {paper.durationMinutes || 0} Minutes
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">

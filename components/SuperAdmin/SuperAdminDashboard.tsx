    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         {[
           { 
             label: 'Total Revenue', 
             value: `${currencySymbol}${formatNumber(metrics.revenue)}`, 
             change: '+14.2%', 
             icon: CreditCard, 
           ...
           { 
             ...
             value: formatNumber(metrics.aiTokens), 
             change: '+31%', 
             icon: Cpu, 
               ...
               {notifications.map((note) => (
                  <div key={note.id} className="p-6 rounded-[2rem] bg-white/5 border border-white/5 hover:bg-white/10 transition-all group relative">
                     ...
                     <div className="flex items-center gap-3 mb-4">
                        <div className={`p-2 rounded-xl ${note.type === 'URGENT' ? 'bg-rose-500/20 text-rose-400' : 'bg-indigo-500/20 text-indigo-400'}`}>
                           {note.type === 'URGENT' ? <ShieldAlert size={16} /> : <Info size={16} />}
                        </div>
                        <span className="text-[11px] font-black text-white uppercase tracking-widest">{note.title}</span>
                     </div>
                     <p className="text-xs text-slate-400 line-clamp-3 leading-relaxed font-medium">{note.message}</p>

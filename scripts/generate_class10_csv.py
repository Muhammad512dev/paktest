...
def generate_improved_class10_csv():
    with open('crawled_class10_drive_files.json', 'r', encoding='utf-8') as f:
        files = json.load(f)
        
    print(f"Loaded {len(files)} Class 10 files from crawl.")
    rows = []
    ...
    for item in files:
        ...
        # Ensure subject is in title for clarity
        if not subject.lower() in raw_title.lower():
            clean_title = f"{subject} Class 10 - {raw_title}"
        else:
...
- Memorize key definitions and bold terms for maximum score."""
    ...
    print(f"Successfully regenerated CSV with {len(rows)} records at: {csv_filename}")
    
    # Also generate JSON import file
    json_filename = 'Class10_Notes_Import.json'
    with open(json_filename, 'w', encoding='utf-8') as f:
        json.dump(rows, f, indent=2, ensure_ascii=False)
    print(f"Successfully regenerated JSON at: {json_filename}")

if __name__ == '__main__':
    generate_improved_class10_csv()

...
// Uses process.cwd() to find the root of the backend folder reliably
...
app.get('/api/contact', authenticate, async (req: any, res: any) => {
  if (req.user.role !== 'SUPER_ADMIN' && req.user.role !== 'SCHOOL_ADMIN') return res.status(403).json({ error: 'Forbidden' });
  try {
    if (prisma.contactQuery) {

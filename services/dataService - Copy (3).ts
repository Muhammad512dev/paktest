...
export const getPublicStats = async () => {
  try {
    ...
    // Return empty stats on failure so the UI doesn't show misleading high numbers
    return { papers: 0, schools: 0, questions: 0 };
  }
};
...
export const getPublicCurriculum = async () => {
  try {
    ...
    return { syllabuses: [], classes: [], subjects: [], chapters: [], sources: [] };
  }
};

export const generatePublicQuiz = async (config: { board: string, grade: string, subject: string, chapter: string, sources: string[], count: number, medium: string }) => {
    try {
        return await apiRequest<Question[]>('/public/quiz/generate', {
            method: 'POST',
...
export const getSystemConfig = async () => {
  try {
    // This endpoint should be public now
    ...
    return { 
      ...
      platformContact: '+1 (555) 000-0000',

...
import { getSystemConfig, updateSystemConfig, updateUser, updateSchool, getUsers, getSchoolById, getCurrentUser, uploadFile } from '../services/dataService';
import { 
  Building2, 
  Palette, 
  Lock, 
...
const Settings: React.FC<SettingsProps> = ({ userRole, onConfigUpdate, onUserUpdate }) => {
  ...
  const allTabs = [
    { 
        ...
        label: 'My Profile', 
        icon: UserIcon, 
        allowedRoles: [UserRole.TEACHER, UserRole.SUPER_ADMIN, UserRole.SCHOOL_ADMIN, UserRole.STUDENT] 
    },
    { 
        id: 'organization', 
    ...
    { id: 'security', label: 'Security', icon: Lock, allowedRoles: [UserRole.SUPER_ADMIN, UserRole.SCHOOL_ADMIN, UserRole.TEACHER, UserRole.STUDENT] },
    { id: 'notifications', label: 'Notifications', icon: Bell, allowedRoles: [UserRole.SUPER_ADMIN, UserRole.SCHOOL_ADMIN, UserRole.TEACHER, UserRole.STUDENT] },
  ];
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>, target: 'SCHOOL' | 'PLATFORM') => {
    const file = e.target.files?.[0];
    if (file) {
      try {
        const url = await uploadFile(file);
        if (target === 'SCHOOL' && schoolData) setSchoolData({ ...schoolData, logo: url });
        else if (target === 'PLATFORM') setSystemConfigState({ ...systemConfig, platformLogo: url });
      } catch (e) {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        const url = await uploadFile(file);
        setAdminProfile({ ...adminProfile, avatar: url });
      } catch (e) {
  ...
  const handleCommitUpdates = async () => {
    try {
        ...
        if (userRole === UserRole.SCHOOL_ADMIN && schoolData) {
            ...
            // Cache branding so next login restores theme instantly
            localStorage.setItem('school_branding', JSON.stringify(branding));
        }
                  ...
                  <>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="md:col-span-2 pt-6 border-t border-gray-100">
                           <h4 className="text-sm font-bold text-gray-900 mb-4">Module Management</h4>
                           <div className="flex items-center justify-between p-5 border border-slate-200 rounded-2xl bg-white shadow-sm">
                               <div>
                                   <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                                      <Globe size={16} className="text-indigo-600"/> Online Exams Module
                                      {(!schoolData.subscriptionPlan || schoolData.subscriptionPlan === 'Starter') && (
                                         <span className="bg-red-100 text-red-600 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-widest">Upgrade Required</span>
                                      )}
                                   </h4>
                                   <p className="text-xs text-slate-500 mt-1">Allow students to take exams online via the student dashboard.</p>
                               </div>
                               <Toggle 
                                   enabled={schoolData.securityConfig?.isOnlineExamsEnabled === true} 
                                   onChange={(val) => {
                                       if (!schoolData.subscriptionPlan || schoolData.subscriptionPlan === 'Starter') {
                                           alert("Your current package (Starter) only supports Paper Generation and PDF Printing. Please upgrade to unlock Online Exams.");
                                           return;
                                       }
                                       setSchoolData({...schoolData, securityConfig: { ...schoolData.securityConfig, isOnlineExamsEnabled: val }})
                                   }} 
                               />
                           </div>
                        </div>

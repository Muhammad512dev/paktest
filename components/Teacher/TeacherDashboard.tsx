...
import { getPapersBySchool, getSchoolById, getPlans } from '../../services/dataService';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ onNavigate, user }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const [schoolData, allPapers, plansData] = await Promise.all([
      ...
      const filtered = allPapers.filter(p => p.author === user.name).slice(0, 5);
  ...
  const stats = [
    { label: 'Papers Created', value: myPapers.length.toString(), icon: FileText, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Exams Slotted', value: '3', icon: Calendar, color: 'text-purple-600', bg: 'bg-purple-50' },
    { label: 'My Classes', value: '4', icon: Users, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Review Score', value: '4.8', icon: TrendingUp, color: 'text-amber-600', bg: 'bg-amber-50' },
  ];

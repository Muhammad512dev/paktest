...
import { 
  ...
  Tag, List, ToggleLeft, FormInput, Database, FileCheck, Loader2, Eye, Filter, Edit2, Check
} from 'lucide-react';
...
const GlobalQuestionBank: React.FC = () => {
  ...
  const handleSaveQuestion = async () => {
      if (!newQuestion.text || !newQuestion.subject || !newQuestion.classLevel) {
          alert("Minimum requirements: Question Text, Subject, and Class.");
          return;
      }
  ...
  const executeSynchronization = async () => {
    ...
    for (let i = 0; i < importRows.length; i++) {
      ...
      // 2. Map CSV row to Question Object
      const q: Question = {
        id: `bulk_${Date.now()}_${i}`,
        text: row.QuestionText_EN || 'Empty Question',
        textUrdu: row.QuestionText_UR || '',
        type: row.Type || 'MCQ',
        ...
        sources: row.Sources ? row.Sources.split('|') : [QuestionSource.MODEL_PAPER],
        source: row.Sources ? row.Sources.split('|')[0] : QuestionSource.MODEL_PAPER,
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50/50 space-y-4">
           ...
           <div className="flex flex-wrap gap-3 items-center">
              ...
              >
                 ...
                 {questionTypesList.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="flex flex-col gap-2">
                           {['MCQ', 'True/False', 'Fill in the Blanks', 'Match Columns', 'Short Answer'].map(t => (
                               <button key={t} onClick={() => downloadTemplate('CSV', t)} className="w-full flex items-center justify-between p-3 border border-gray-200 rounded-xl hover:bg-gray-50 transition-all text-left">
                    ...
                    <div className="space-y-4">
                       ...
                       <div 
                        ...
                        >
                          ...
                          <p className="text-xs font-bold text-gray-700">Choose CSV File</p>
                          ...
                          return (
                             <tr key={i} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4 border-r border-gray-100 max-w-md">
                                   <p className="text-sm font-semibold text-gray-900 truncate">{row.QuestionText_EN}</p>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                       ...
                       <div className="flex flex-col gap-2 max-h-[180px] overflow-y-auto pr-2 custom-scrollbar">
                          {['MCQ', 'True/False', 'Fill in the Blanks', 'Match Columns', 'Short Answer'].map(t => (
                             <button key={t} onClick={() => downloadTemplate('CSV', t, true)} className="w-full flex items-center justify-between p-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all text-left group">
                                <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-4">
                       ...
                       <div 
                        ...
                        >
                          ...
                          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Drop CSV File</p>
              ...
              {formStep === 'TYPE' ? (
                <div className="p-8 grid grid-cols-2 md:grid-cols-4 gap-4">
                   {questionTypesList.map(type => (
                      <div 
                         key={type.id} 
                         onClick={() => { setNewQuestion({...newQuestion, type: type.id}); setFormStep('CONTENT'); }}
                ...
                <div className="flex-1 overflow-y-auto">
                    <div className="p-6 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           ...
                           <div className="space-y-2">
                               <label className="text-xs font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2">
                                <ImageIcon size={14}/> Diagram / Image Link
                               </label>
                               <div className="relative">
                                  <input type="text" value={newQuestion.imageUrl || ''} onChange={e => setNewQuestion({...newQuestion, imageUrl: e.target.value})} className="w-full pl-4 pr-4 py-3 border border-slate-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 shadow-sm" placeholder="Paste image link..." />
                               </div>
                           </div>
                        </div>
                        ...
                        {newQuestion.type === 'Fill in the Blanks' && (
                           <div className="space-y-6 pt-6 border-t border-slate-100">
                              <h5 className="font-bold text-slate-800 text-sm uppercase tracking-widest">Expected Answers</h5>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 <div className="space-y-2">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Answer (EN)</label>
                                    <input value={newQuestion.correctAnswer || ''} onChange={e => setNewQuestion({...newQuestion, correctAnswer: e.target.value})} className="w-full border border-slate-200 p-3 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Enter English answer..." />
                                 </div>
                                 {isBilingualMode && (
                                    <div className="space-y-2">
                                       <label className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest text-right block">درست جواب (اردو)</label>
                                       <input dir="rtl" value={newQuestion.correctAnswerUrdu || ''} onChange={e => setNewQuestion({...newQuestion, correctAnswerUrdu: e.target.value})} className="w-full border border-indigo-50 p-3 rounded-xl font-urdu text-xl text-right focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="جواب لکھیں..." />
                                    </div>
                                 )}
                              </div>
                              <div className="p-3 bg-indigo-50 text-indigo-700 text-xs rounded-lg border border-indigo-100 flex items-start gap-2">
                                 <Info size={14} className="shrink-0 mt-0.5" />
                                 <p>For blanks, use <strong>______</strong> or <strong>(blank)</strong> in the question text above to indicate the gap.</p>
                              </div>
                           </div>
                        )}

                        {newQuestion.type === 'Diagram Based' && (
                           <div className="space-y-6 pt-6 border-t border-slate-100">
                              <h5 className="font-bold text-slate-800 text-sm uppercase tracking-widest flex items-center gap-2">
                                 <ImageIcon size={18} className="text-purple-600"/> Diagram Specification
                              </h5>
                              <div 
                                 className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50 group hover:border-purple-300 transition-all cursor-pointer relative min-h-[240px]"
                                 onClick={() => diagramFileInputRef.current?.click()}
                              >
                                 {newQuestion.imageUrl ? (
                                    <div className="relative w-full max-w-md animate-in fade-in zoom-in-95 duration-300">
                                       <img src={newQuestion.imageUrl} className="w-full h-auto rounded-xl shadow-lg border border-slate-200" alt="Diagram Preview" />
                                       <button 
                                          onClick={(e) => { e.stopPropagation(); setNewQuestion({...newQuestion, imageUrl: ''}); }} 
                                          className="absolute -top-3 -right-3 bg-red-500 text-white p-2.5 rounded-full shadow-xl hover:bg-red-600 transition-all transform hover:scale-110"
                                       >
                                          <Trash2 size={18}/>
                                       </button>
                                    ...
                                    <>
                                       ...
                                       <p className="text-sm font-bold text-slate-700">Click or Drag to Upload Diagram</p>
                                       <p className="text-xs text-slate-400 mt-2 font-medium">Supports JPG, PNG, SVG (Max 5MB)</p>
                                    </>
                                 )}
                                 <input type="file" ref={diagramFileInputRef} className="hidden" accept="image/*" onChange={handleDiagramUpload} />
                              </div>
                              <div className="p-4 bg-purple-50 text-purple-700 text-xs rounded-xl border border-purple-100 flex items-start gap-3">
                                 <AlertCircle size={18} className="shrink-0 mt-0.5" />
                                 <div>
                                    <p className="font-bold mb-1">Diagram Labeling Instructions</p>
                                    <p className="leading-relaxed opacity-80">Use the main question text area to provide instructions (e.g. "Label parts A, B, and C in the diagram below"). The uploaded file will be rendered directly below the question text.</p>
                                 </div>
                              </div>
                           </div>
                        )}

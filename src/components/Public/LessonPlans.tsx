...
import { Search, FileText, Download, Filter, BookOpen, Layers, GraduationCap, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const LessonPlans: React.FC = () => {
  ...
  const isFiltered = Boolean(selectedBoard || selectedClass || selectedSubject || searchTerm);
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchTerm, selectedBoard, selectedClass, selectedSubject, isFiltered, userCustomLimit]);
              ...
              {totalFilteredCount > 0 && (
                <div className="flex flex-col sm:flex-row justify-between items-center gap-2 px-1 text-xs text-slate-500 font-semibold">
                  <div>
                    ...
                    </span> of <span className="font-bold text-slate-800">{totalFilteredCount}</span> lesson plans
